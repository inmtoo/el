<template>
  <main class="main">
    <div class="container">
      <UITheBreadcrumbs :items="breadcrumbs"></UITheBreadcrumbs>
    </div>

    <div class="container">
      <div class="article">
        <div class="article__content">
          <h1>О компании Vlada Group</h1>
          <p>
            VLADA GROUP — уникальное место, благодаря которому создается
            премиальный визуальный контент: аренда камер и кино-оптики ведущих
            брендов, профессиональный свет и LED-экраны с разрешением до 8K, а
            также трансформируемая площадка более 300 м² для съёмок и
            мероприятий. Команда с более чем 25-летним опытом реализует проекты
            любого масштаба — от рекламы и клипов до кинопроизводства и
            событийных решений, предоставляя технику, логистику и сопровождение
            на площадке.
          </p>
          <p><img src="/images/about.jpg" /></p>
        </div>
        <div class="article__shared">
          <div class="article__shared-title">Поделиться</div>
          <div class="article__shared-links">
            <a href="#" @click.prevent="onShareClick">
              <svg
                width="32"
                height="32"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <g id="SVGRepo_bgCarrier" stroke-width="0" />

                <g
                  id="SVGRepo_tracerCarrier"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                />

                <g id="SVGRepo_iconCarrier">
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M19.6495 0.799565C18.4834 -0.72981 16.0093 0.081426 16.0093 1.99313V3.91272C12.2371 3.86807 9.65665 5.16473 7.9378 6.97554C6.10034 8.9113 5.34458 11.3314 5.02788 12.9862C4.86954 13.8135 5.41223 14.4138 5.98257 14.6211C6.52743 14.8191 7.25549 14.7343 7.74136 14.1789C9.12036 12.6027 11.7995 10.4028 16.0093 10.5464V13.0069C16.0093 14.9186 18.4834 15.7298 19.6495 14.2004L23.3933 9.29034C24.2022 8.2294 24.2022 6.7706 23.3933 5.70966L19.6495 0.799565ZM7.48201 11.6095C9.28721 10.0341 11.8785 8.55568 16.0093 8.55568H17.0207C17.5792 8.55568 18.0319 9.00103 18.0319 9.55037L18.0317 13.0069L21.7754 8.09678C22.0451 7.74313 22.0451 7.25687 21.7754 6.90322L18.0317 1.99313V4.90738C18.0317 5.4567 17.579 5.90201 17.0205 5.90201H16.0093C11.4593 5.90201 9.41596 8.33314 9.41596 8.33314C8.47524 9.32418 7.86984 10.502 7.48201 11.6095Z"
                    fill="#ff8900"
                  />
                  <path
                    d="M7 1.00391H4C2.34315 1.00391 1 2.34705 1 4.00391V20.0039C1 21.6608 2.34315 23.0039 4 23.0039H20C21.6569 23.0039 23 21.6608 23 20.0039V17.0039C23 16.4516 22.5523 16.0039 22 16.0039C21.4477 16.0039 21 16.4516 21 17.0039V20.0039C21 20.5562 20.5523 21.0039 20 21.0039H4C3.44772 21.0039 3 20.5562 3 20.0039V4.00391C3 3.45162 3.44772 3.00391 4 3.00391H7C7.55228 3.00391 8 2.55619 8 2.00391C8 1.45162 7.55228 1.00391 7 1.00391Z"
                    fill="#ff8900"
                  />
                </g>
              </svg>
            </a>
          </div>
        </div>
      </div>
    </div>
  </main>
</template>

<script setup lang="ts">
import { share } from "~/helpers/utils";

const breadcrumbs = [
  { title: "Главная", link: "/" },
  { title: "О компании", link: "" },
];

useHead({
  title: "Vlada Group — о компании",
  meta: [
    {
      name: "keywords",
      content: "о компании Vlada Group",
    },
    {
      name: "description",
      content:
        "Мы ваш партнер в вопросах техники и пространства для съемок кино, рекламы, клипов, а также проведения мероприятий.",
    },
    {
      property: "og:title",
      content: "Vlada Group — о компании",
    },
    {
      property: "og:description",
      content:
        "Мы ваш партнер в вопросах техники и пространства для съемок кино, рекламы, клипов, а также проведения мероприятий.",
    },
    { property: "og:image", content: "/images/about-vlada.jpg" },
  ],
});

function scrollToElem(className: string, behavior: ScrollBehavior = "smooth") {
  const el = document.querySelector(className);
  if (el) {
    const offsetTop = el.getBoundingClientRect().top + window.scrollY;
    window.scrollTo({
      top: offsetTop,
      behavior,
    });
  } else {
    console.warn(`Element with class "${className}" not found.`);
  }
}

function onShareClick() {
  share("Vlada", "Vlada", "/images/about-vlada.jpg");
}
</script>
